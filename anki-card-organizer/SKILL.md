...
- In every future card, render each actual fixed expression, collocation, or reusable grammatical structure in red wherever it appears, including the original text and explanations. Color only the exact expression or structure, not the entire sentence, translation, label, or title. On explanatory bullet lists, bold the core judgment or learning point in each bullet while leaving supporting detail at normal weight. A span that is both a key point and a fixed structure may be both red and bold. Use the structured `parts` format described in the [batch format](references/ankiconnect.md#batch-format); never place raw HTML in card text.
...
- Front: preserve the expressions supplied by the user, in their original order, with one expression per left-aligned bullet on its own line (two bullets when comparing two expressions). Do not use a comparison table, translations, corrections, or answer hints on the expression lines; the required red styling of fixed expressions and structures is the only added emphasis. Keep any title separate and preserve its alignment.
